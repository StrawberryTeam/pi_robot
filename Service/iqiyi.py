...
class iqiyi():
    '''
    爱奇艺 http://list.iqiyi.com/www/15/-------------11-2-1---.html
    '''
    # 平台标识
    _platform = 1
    ...
    def getMovieCate(self, movie):
        ...
        for lc in lcontent:
            ...
            if "总播放量：" in lc:
                ...
                movieInfo['allplaynum'] = common.getRealPlayNum(movieInfo['allplaynum_txt'])
        for rc in rcontent:
            rc = rc.get_text()
            if '集数：' in rc:
                # 当前集数
...
if __name__ == "__main__":
    i = iqiyi()
    for link in i._cateLink:
        i.getCateContent(link)
    print('Cate end')
